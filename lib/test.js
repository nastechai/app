/**
 * Basic tests for Nastech-Termux
...
async function runTests() {
  console.log('\n🧪 Nastech-Termux Tests\n');

  // File existence tests
  console.log('📁 File Structure:');

  test('bin/nastech exists', () => {
    assert.ok(existsSync(join(projectRoot, 'bin/nastech')));
  });
  ...
  test('package.json has required fields', () => {
    ...
    assert.ok(pkg.name === 'nastech-termux');
    assert.ok(pkg.version);
    assert.ok(pkg.main);
    assert.ok(pkg.bin);
    assert.ok(pkg.bin.nastech);
  });
